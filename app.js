...
function map(){
 app.innerHTML=`<section class="card"><p class="eyebrow">HYPOTHÈSES DU QUESTIONNAIRE</p><h2>Émotions et états distingués</h2><div class="map">${Object.entries(STATES).map(([k,s])=>`<button data-k="${k}"><span class="icon">${s.icon}</span><strong>${esc(s.name)}</strong><small>${esc(s.kind)}</small></button>`).join('')}</div></section>`;
 document.querySelectorAll('[data-k]').forEach(b=>b.onclick=()=>{S.mapKey=b.dataset.k;setView('mapDetail')});
}
function mapDetail(){
 const s=STATES[S.mapKey];
 const qs=ALL_QUESTIONS.filter(q=>q.w&&q.w[S.mapKey]).sort((a,b)=>(b.w[S.mapKey]-a.w[S.mapKey]));
 app.innerHTML=`<section class="card"><button class="secondary" onclick="setView('map')">← Retour</button><p class="eyebrow">${esc(s.kind)}</p><h2>${s.icon} ${esc(s.name)}</h2><p>${esc(s.function)}</p>
 <h3>Questions les plus liées à cette hypothèse</h3>${qs.slice(0,8).map(q=>`<div class="evidence"><strong>${esc(q.q)}</strong><p class="muted">${q.tier==='core'?'Fiche initiale':q.tier==='research'?'Complément de recherche':'Contexte'}</p></div>`).join('')}</section>`;
}
